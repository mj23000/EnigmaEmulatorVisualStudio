...
namespace SRP_Enigma
{
	public partial class Form1 : Form
	{
		...
		{
			rotor1RotorI.Checked = true;
			rotor2RotorII.Checked = true;
			rotor3RotorIII.Checked = true;
		...
		void krypteringAlgoritme()
		{
			...
			void rotorOffsetRotorHoejre()
			{
				if ((taellerFindBogstav + rotorHoejreStatusInt) > 25) taellerFindBogstav = 0;
			}

			void rotorOffsetRotorMidt()
			{
				while (bogstav.Equals(ETW[taellerFindBogstav]) == false) taellerFindBogstav++;

				if ((taellerFindBogstav + rotorMidtStatusInt) > 25) taellerFindBogstav = 0;
			}

			void rotorOffsetRotorVenstre()
			{
				while (bogstav.Equals(ETW[taellerFindBogstav]) == false) taellerFindBogstav++;
			...
			void krypterTrinEt()
			{
				...
				rotorOffsetRotorHoejre();
				checkRotorStatusAfslut();
				krypterCharRotorHoejre();

				rotorOffsetRotorMidt();
				krypterCharRotorMidt();

				rotorOffsetRotorVenstre();
				krypterCharRotorVenstre();
			}

			void krypterReflektor()
			{
				while (bogstav.Equals(reflektorIndgang[taellerFindBogstav]) == false) taellerFindBogstav++;
